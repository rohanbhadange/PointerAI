...
public sealed class UpdateService
{
    ...
    {
        httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("PointerAI-Updater/1.0");
    }
    ...
    public async Task<UpdateCheckResult> CheckForUpdatesAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var installerAsset = release?.Assets?
                .FirstOrDefault(asset => asset.Name.Contains("ClickyCloneSetup", StringComparison.OrdinalIgnoreCase) &&
        ...
        CancellationToken cancellationToken)
    {
        ...
        var installerPath = Path.Combine(updateDirectory, $"ClickyCloneSetup-{version}.exe");
