...
public sealed class WorkerSetupRunner
{
    ...
    public string PrepareDeployPackage()
    {
        ...
        var bundledWorker = Path.Combine(appBase, "SetupWorker", "clickyclone-worker.js");
        if (!File.Exists(bundledWorker))
        {
            bundledWorker = Path.Combine(appBase, "worker", "clickyclone-worker.js");
        }

        if (!File.Exists(bundledWorker))
        {
            bundledWorker = Path.GetFullPath(Path.Combine(appBase, "..", "..", "..", "..", "..", "worker", "clickyclone-worker.js"));
        }
        ...
        File.Copy(bundledWorker, Path.Combine(deployRoot, "clickyclone-worker.js"), overwrite: true);
        var workerName = "pointerai-" + Guid.NewGuid().ToString("N")[..8];
