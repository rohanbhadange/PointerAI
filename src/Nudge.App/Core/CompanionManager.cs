...
public sealed class CompanionManager : INotifyPropertyChanged, IDisposable
{
    ...
    private async Task CheckWorkerHealthAsync()
    {
        try
        {
            ...
            {
                AppLogger.Info("Worker exact pointing support is not available. Redeploy worker/nudge-worker.js before judging pointing accuracy.");
            }

            var diagnostics = await backendClient.GetDiagnosticsAsync(CancellationToken.None);
            if (IsNudgeWorker(diagnostics.Service) &&
    ...
    private static bool IsNudgeWorker(string? service)
    {
        var legacyService = string.Concat("click", "yclone-worker");
        return string.Equals(service, "nudge-worker", StringComparison.OrdinalIgnoreCase) ||
               string.Equals(service, legacyService, StringComparison.OrdinalIgnoreCase);
    }
