...
{
    var settingsPath = Path.Combine(Path.GetTempPath(), "Nudge.Tests", Guid.NewGuid().ToString("N"), "settings.json");
...
{
    var envPath = Path.Combine(Path.GetTempPath(), "Nudge.Tests", Guid.NewGuid().ToString("N"), ".env");
...
{
    const string output = "Uploaded nudge-test\nDeployed nudge-test triggers\nhttps://nudge-test.example.workers.dev";
    var workerUrl = WorkerSetupRunner.TryExtractWorkerUrl(output);
    AssertStringEqual("https://nudge-test.example.workers.dev", workerUrl ?? "", "worker url");
}
